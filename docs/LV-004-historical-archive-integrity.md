...
- **F1 — `app/api/aviation/snapshot/route.js` has the same silent-fallback shape as the hotels endpoint had.** If its own upstream ADSB.lol call fails, it returns HTTP 200 with zeroed-out counts and no distinguishing signal (unlike hotels, which at least sets `source: 'fallback'`). The cron route would record `sourceFreshness.aviation: 'ok'` for a run that was actually degraded. **Follow-up: LV-005.**
- **F2 — Falsy-value coercion (`|| 450`, `|| totalArrivals`) treats a legitimate zero as "missing."** `d.flight_arrivals_total || 450` and `osData.length || totalArrivals` cannot distinguish a real zero-arrivals reading from an absent one. Fixing this touches the same calculation lines this ticket was explicitly told not to redesign; it deserves its own reviewed change. **Follow-up: LV-006.**
- **F3 — Two independent, differently-implemented private-jet classifiers exist**: `lib/flightUtils.js`'s `classifyAircraft()` (used by the public `/api/aviation/snapshot` dashboard) and an inline heuristic in the cron route (`callsign.startsWith('N') && length <= 6`, used for the archived CVI). These can disagree for reasons that have nothing to do with real-world conditions. **Follow-up: LV-007.**
