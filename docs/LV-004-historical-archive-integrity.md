...
| `private_jet_activity_index` | Raw ADSB.lol fetch (independent of `/api/aviation/snapshot`) | `(private aircraft ÷ total descending aircraft) ÷ 0.08` — a baseline-relative index, can exceed 1. **Canonical field, added by LV-008.** See `docs/LV-008-private-jet-metric-naming.md` |
| `private_jet_count` (deprecated) | Same computation as `private_jet_activity_index` | Numerically identical to `private_jet_activity_index` on every record written from LV-008 onward — a **compatibility alias**, not a literal count despite the name; see Finding F5 |
| `private_jet_index_normalized` | `private_jet_activity_index` | `min(100, private_jet_activity_index × 50)` — the actual CVI input. **Added by LV-004; unchanged by LV-008** |
...
- **F5 — TRANSITIONED to a compatibility deprecation by LV-008**, not a hard rename (a breaking rename was correctly avoided). `private_jet_activity_index` is now the canonical, semantically accurate field, dual-written alongside the still-present, deprecated `private_jet_count` for backward compatibility. See `docs/LV-008-private-jet-metric-naming.md` for the reader-fallback contract and eventual-removal prerequisites.
- **F6 — RESOLVED by LV-009.** `weeklyMovementBrief` now declares `timeZone: "America/Los_Angeles"` explicitly. Confirmed empirically (not assumed) against the installed `firebase-functions` v6.6.0 source that the prior plain-string `onSchedule` form never sent a `timeZone` to the deployed endpoint at all, so it was actually executing at 08:00 UTC (00:00 PST / 01:00 PDT), not the evidently-intended Monday-morning Las Vegas report. See `docs/LV-009-weekly-brief-timezone.md`.
