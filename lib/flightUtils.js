...
export function classifyAircraft(type, callsign) {
    ...
    // LV-007: (type || '').toUpperCase() throws if type is a non-string
    // truthy value (an object, a number) -- a malformed upstream record
    // could crash the entire aircraft-processing loop, not just this one
    // record. Coercing only real strings (and treating anything else as
    // absent) keeps this function pure and total for any input shape.
    const cleanType = (typeof type === 'string' ? type : '').toUpperCase().trim();
    const cleanCallsign = (typeof callsign === 'string' ? callsign : '').toUpperCase().trim();
...
/**
 * The single authoritative private-jet determination used by both the
 * live aviation dashboard and the archived daily snapshot pipeline
 * (LV-007). A thin boolean wrapper around classifyAircraft rather than a
 * second implementation -- there is exactly one place aircraft evidence
 * (ICAO type, callsign) is turned into a private/not-private decision.
 * @param {string} type - Aircraft ICAO type code
...
export function computeNetFlow(inboundCount, outboundCount) {
    const value = inboundCount - outboundCount;
    let direction = 'BALANCED';
    if (value > 5) direction = 'INBOUND';
    else if (value < -5) direction = 'OUTBOUND';
    const ratio = outboundCount > 0 ? inboundCount / outboundCount : inboundCount;
    return { value, direction, ratio };
}

export function computeArrivalRate(flightCount, timeWindowMinutes) {
    if (timeWindowMinutes === 0) return 0;
    return Math.round((flightCount / timeWindowMinutes) * 60);
}

export function computeAircraftMixScore(flights) {
    ...
    flights.forEach(f => {
        ...
        if (cat === 'Commercial') {
            if (['A332','A333','A339','A359','A35K','B763','B764','B772','B77W','B788','B789','B78X'].includes(type)) {
                score += 3;
            } else {
                score += 2;
            }
    ...
    return score / flights.length;
}

export function computePrivateJetIndex(privateCount, totalCount, baseline = 0.08) {
    if (totalCount === 0) return 1.0;
    const currentRatio = privateCount / totalCount;
    return currentRatio / baseline;
}

export function getDemandPressureLabel(netFlow, arrivalRate, baselineRate = 20) {
    if (netFlow.direction === 'INBOUND' && arrivalRate > baselineRate * 1.5) return 'SURGING';
    if (netFlow.direction === 'INBOUND' && arrivalRate > baselineRate) return 'HIGH';
    if (netFlow.direction === 'OUTBOUND' && arrivalRate < baselineRate * 0.5) return 'DECLINING';
    if (netFlow.direction === 'OUTBOUND') return 'LOW';
    return 'MODERATE';
}

export function computeEstimatedDailyPax(currentHourPax, currentHour) {
    let hourlyFactor = 0.04;
    if (currentHour >= 13 && currentHour <= 18) hourlyFactor = 0.08;
    else if (currentHour >= 7 && currentHour <= 12) hourlyFactor = 0.05;
    else if (currentHour >= 19 && currentHour <= 23) hourlyFactor = 0.05;
    else hourlyFactor = 0.02;

    if (hourlyFactor === 0) return currentHourPax * 24;
    return Math.round(currentHourPax / hourlyFactor);
}
