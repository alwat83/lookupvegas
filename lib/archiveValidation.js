...
// LV-008: bumped from 'v2' -- a new persisted field (private_jet_activity_index)
// is being introduced, the same class of change that justified 'v2' in the
// first place (LV-004). Consistent with that precedent, not a new policy.
...
// Only guaranteed on documents written after LV-008 (schema_version 'v3').
// private_jet_count remains present and numerically identical on these
// documents too -- this is an added canonical alias, not a replacement.
const V3_ONLY_FIELDS = ['private_jet_activity_index'];

const VALID_STATUSES = ['success', 'partial', 'failed'];

const DATE_FORMAT = /^\d{4}-\d{2}-\d{2}$/;

function detectSchemaGeneration(doc) {
    if (doc.schema_version === 'v3' || V3_ONLY_FIELDS.some(f => doc[f] !== undefined)) return 'v3';
    if (doc.schema_version === 'v2' || V2_ONLY_FIELDS.some(f => doc[f] !== undefined)) return 'v2';
    return 'v1-legacy';
}
...
export function validateDocumentSchema(doc) {
    ...
    const schemaGeneration = detectSchemaGeneration(doc);

    if (schemaGeneration === 'v2' || schemaGeneration === 'v3') {
        for (const field of V2_ONLY_FIELDS) {
            if (doc[field] === undefined || doc[field] === null) {
                issues.push({ field, problem: `missing on a schema_version ${schemaGeneration} document (inconsistent schema)` });
            }
        }
    }

    if (schemaGeneration === 'v3') {
        for (const field of V3_ONLY_FIELDS) {
            if (doc[field] === undefined || doc[field] === null) {
                issues.push({ field, problem: `missing on a schema_version v3 document (inconsistent schema)` });
            }
        }
    }

    // LV-008: a document is never falsely failed for predating the
    // canonical field (that's what schemaGeneration above already
    // handles) -- but if BOTH the canonical and legacy fields are present
    // and materially disagree, that's a real integrity problem worth
    // surfacing, not silently picking one.
    const jetReading = readPrivateJetActivityIndex(doc);
    if (jetReading.reason === 'disagreement') {
        issues.push({ field: 'private_jet_activity_index', problem: jetReading.issue });
    }

    return { valid: issues.length === 0, issues, schemaGeneration };
}

// LV-008: private_jet_count does not store a literal count -- it is a
// baseline-relative index ((privateAircraft/totalDescending)/0.08) that
// can legitimately exceed 1. private_jet_activity_index is the
// semantically accurate canonical name for the same value, dual-written
// alongside the deprecated private_jet_count for backward compatibility.
// This reader is the single place that reconciles the two: prefer the
// canonical field, fall back to the legacy one, and surface -- never
// silently resolve -- a material disagreement between them.
export function readPrivateJetActivityIndex(doc, tolerance = 0.05) {
    const canonical = doc.private_jet_activity_index;
    const legacy = doc.private_jet_count;
    const canonicalValid = typeof canonical === 'number' && Number.isFinite(canonical);
    const legacyValid = typeof legacy === 'number' && Number.isFinite(legacy);

    if (canonicalValid && legacyValid) {
        const diff = Math.abs(canonical - legacy);
        if (diff > tolerance) {
            return {
                value: null,
                source: null,
                ok: false,
                reason: 'disagreement',
                issue: `private_jet_activity_index (${canonical}) and private_jet_count (${legacy}) disagree by ${diff}, exceeding tolerance ${tolerance}`,
            };
        }
        return { value: canonical, source: 'canonical', ok: true, reason: null, issue: null };
    }
    if (canonicalValid) {
        return { value: canonical, source: 'canonical', ok: true, reason: null, issue: null };
    }
    if (legacyValid) {
        return { value: legacy, source: 'legacy', ok: true, reason: null, issue: null };
    }
    return {
        value: null,
        source: null,
        ok: false,
        reason: 'missing',
        issue: 'neither private_jet_activity_index nor private_jet_count is present and finite',
    };
}
...
const RANGE_RULES = {
    ...
    private_jet_activity_index: [0, null],
};
